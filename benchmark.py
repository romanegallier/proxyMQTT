 ...
 class publishThread (Thread):
    ...
    def run(self):
        try:
            ...
            client.loop_start
            client.connect(BROKER_ADRESS,BROKER_PORT, KEEP_ALIVE) 
            print(self.clientId + " publish on topic:" + self.topic +' :'+self.payload)
            client.publish(self.topic, self.payload, self.qos)
            time.sleep(self.stayAlive) 
            client.loop_stop() 
        except BaseException as e:
            print ("Error in publishThread with topic "+ self.topic+":\n"+ str(e))
...
def generation_arbre(height = 0, name= 'a', nb_max_child = 5, nb_min_child =0, height_max=5, height_min =0 ):
    if (height>= height_max):
        return Arbre ('l_' + name)
    else:
        if (height <height_min and nb_min_child < 1 ) : nb_child = randint(1, nb_max_child)
        else : nb_child = randint(nb_min_child, nb_max_child)
        
        if nb_child >0:
            a  = Arbre('n_' + name)
            for i in range(0, nb_child):
                a.ajoute (generation_arbre(height+1, name +chr(ord('a')+i) , nb_max_child, nb_min_child, height_max, height_min))
        else: a= Arbre ('l_' + name)
    return a

def generate_topic_s(a, root='', wildcard=False, first = True):
    l =[]
    l.append(root + a.racine)
    if root != '' and wildcard== False and first:
        l.append(root + '+')
        l.append(root + '#')
        
    
    for f in a :
        l+=generate_topic_s(f, root + a.racine+ '/', first=first)
        l+=generate_topic_s(f, root + '+/', wildcard or root =='', first= first)
        first = False
    return l
...
def generate_topic_p(a, root=''):
    ...
    l.append(root + a.racine)
    for f in a :
        l+=generate_topic_p(f, root + a.racine+ '/')
    return l

def generateMessages(nb_message, longueur):
